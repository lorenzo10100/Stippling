...
def Centroids(sites, pd, bounds, step):
    """
    La seguente funzione calcola i centroidi di ogni sito, ritornando un array
    di centroidi. Per ogni punto dell'immagine si calcola il sito più vicino, applica la funzione di densità
    e viene pesato con essa.
    @param sites: array di siti
    @param pd: funzione di densità
    @param bounds: dimensioni dell'immagine
    @param step: passo
    @return array di centroidi
    """
    siteCentroids = dict()
